const CategoryIllustrations = () => {
  ...
  const [bgColor, setBgColor] = React.useState(colors.bg);

  return (
    <div className="min-h-screen p-8" style={{ backgroundColor: bgColor }}>
      <div className="flex justify-center gap-4 mb-6">
        <button 
          onClick={() => setBgColor(colors.bg)} 
          className="px-4 py-2 rounded-lg text-sm font-medium transition-all"
          style={{ 
            backgroundColor: bgColor === colors.bg ? colors.teal : 'rgba(255,255,255,0.1)',
            color: bgColor === colors.bg ? colors.bg : colors.teal
          }}
        >
          Dark Background
        </button>
        <button 
          ...
          className="px-4 py-2 rounded-lg text-sm font-medium transition-all"
          style={{ 
            backgroundColor: bgColor === '#FFFFFF' ? colors.orange : 'rgba(255,255,255,0.1)',
            color: bgColor === '#FFFFFF' ? '#FFF' : colors.mint
          }}
        >
          White Background
        </button>
        <button 
          ...
          className="px-4 py-2 rounded-lg text-sm font-medium transition-all"
          style={{ 
            backgroundColor: bgColor === '#F3F4F6' ? colors.green : 'rgba(255,255,255,0.1)',
            color: bgColor === '#F3F4F6' ? colors.bg : colors.mint
          }}
        >
          Light Gray
        </button>
      </div>
      <h1 className="text-3xl font-bold text-center mb-2" style={{ color: bgColor === colors.bg ? colors.teal : colors.bg }}>Event Services Categories</h1>
      <p className="text-center mb-8" style={{ color: bgColor === colors.bg ? colors.mint : '#6B7280' }}>Transparent SVG illustrations - preview on different backgrounds</p>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 max-w-7xl mx-auto">
        {categories.map((cat, index) => (
          <div 
            key={index} 
            className="rounded-2xl p-4 cursor-pointer group transition-all duration-300 hover:scale-105"
            style={{ backgroundColor: bgColor === colors.bg ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.03)' }}
          >
            <div className="w-full aspect-square mb-3 rounded-xl overflow-hidden">
              {cat.illustration}
            </div>
            <h3 className="text-sm font-medium text-center" style={{ color: bgColor === colors.bg ? colors.mint : colors.bg }}>{cat.name}</h3>
          </div>
        ))}
      </div>
    </div>
