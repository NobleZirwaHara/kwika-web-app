...
export default function ProductsEdit({ admin, product, catalogues, providers }: Props) {
  const { data, setData, post, processing, errors } = useForm({
    _method: 'PUT',
    service_provider_id: product.service_provider_id.toString(),
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <div className="mt-2 space-y-4">
                  {/* Existing Image */}
                  {product.primary_image && keepExistingImage && !imagePreview && (
                    ...
                    <div className="flex items-center justify-center w-48 h-48 border-2 border-dashed rounded-lg cursor-pointer hover:border-primary">
                      <label htmlFor="primary_image" className="cursor-pointer text-center p-4">
                        ...
                        <span className="text-sm text-muted-foreground">
                          {product.primary_image ? 'Change image' : 'Click to upload'}
                        </span>
                      </label>
                    </div>
                  )}
